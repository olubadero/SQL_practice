...
-- adding and deducting days and time
...
SELECT o.account_id, a.name as customer, SUM(o.total_amt_usd) as total_spend, CASE WHEN SUM(o.total_amt_usd) > 200000 THEN 'Top Level' WHEN SUM(o.total_amt_usd) BETWEEN 100000 AND 200000 THEN 'Mid-level' ELSE 'Low Level' END as Customer_Levels
FROM orders as o
JOIN accounts as a
ON o.account_id = a.id
...
-- convert a string date column to date format, use this '%d/%m/%Y' to identify
-- which position is day month or year as written in the column 
...
LEAD(total_sum) OVER (ORDER BY occurred_at) AS lead, LEAD(total_sum) OVER (ORDER BY occurred_at) - total_sum AS lead_difference
FROM (
SELECT occurred_at,
